...
const EmptyState: React.FC<{
	...
	<div className='flex flex-col items-center justify-center h-full select-none px-6'>
		...
		<div className='text-[11px] text-zinc-600 mb-5 max-w-[320px] text-center'>
			Forge can understand the codebase, edit files, run commands and tests, use the browser, create designs, and build Work Mode automations.
		</div>
		<div className='flex flex-wrap gap-1.5 justify-center max-w-[420px]'>
			{[
				...
				<button
					...
					className='px-2.5 py-1.5 rounded-lg border border-zinc-800/60 bg-zinc-900/40 text-[11px] text-zinc-500 hover:bg-zinc-800/40 hover:text-zinc-300 hover:border-zinc-700/60 transition-colors cursor-pointer'
				>
					{suggestion}
				</button>
			))}
		...
		<button
			...
			className='mt-5 flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-zinc-800/40 hover:bg-zinc-700/40 border border-zinc-700/30 text-[10px] text-zinc-600 font-mono transition-colors cursor-pointer'
		>
			<Command size={10} />
			<span>/</span>
...
const MessageActions: React.FC<{
	...
	return (
		<div className='absolute right-3 top-2 hidden group-hover:flex items-center gap-0.5 rounded-md border border-zinc-800/60 bg-zinc-950/90 p-0.5 shadow-lg'>
			{onCopy && (
				<button type='button' onClick={onCopy} title='Copy message' aria-label='Copy message' className='w-6 h-6 flex items-center justify-center rounded text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800/80 transition-colors'>
					<Copy size={11} />
				</button>
			)}
			{onDuplicateThread && (
				<button type='button' onClick={onDuplicateThread} title='Duplicate thread' aria-label='Duplicate thread' className='w-6 h-6 flex items-center justify-center rounded text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800/80 transition-colors'>
					<GitFork size={11} />
				</button>
			)}
			{onRevert && (
				<button type='button' onClick={onRevert} title='Revert to here' aria-label='Revert to here' className='w-6 h-6 flex items-center justify-center rounded text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800/80 transition-colors'>
					<RotateCcw size={11} />
				</button>
			)}
		</div>
	...
	<div className='group relative flex gap-3 py-3 px-4 hover:bg-zinc-900/10 transition-colors'>
		...
		<div className='flex-1 min-w-0'>
			<div className='flex items-center gap-2 mb-0.5'>
				...
				<span className='text-[9px] text-zinc-700'>{new Date(message.timestamp).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}</span>
			</div>
		...
		<div className='group relative flex gap-3 py-3 px-4 hover:bg-zinc-900/10 transition-colors'>
			...
			<div className='flex-1 min-w-0'>
				<div className='flex items-center gap-2 mb-1'>
					<span className='text-[11px] font-medium text-zinc-400'>Forge</span>
					<span className='text-[9px] text-zinc-700'>{new Date(message.timestamp).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}</span>
				</div>
				{hasActiveWork && (
				...
				{isStreaming && (
					<div className='flex items-center gap-1.5 mt-2 text-zinc-600'>
						...
						<span className='text-[10px] animate-pulse'>Working…</span>
					</div>
				)}
				{!isStreaming && message.content && onFeedback && (
					<div className='flex items-center gap-0.5 mt-2'>
						<button type='button' onClick={() => submitFeedback('positive')} title='Helpful response' className={`w-6 h-6 flex items-center justify-center rounded transition-colors ${feedback === 'positive' ? 'text-emerald-400 bg-emerald-500/10' : 'text-zinc-700 hover:text-zinc-400 hover:bg-zinc-800/50'}`}>
							<ThumbsUp size={11} />
						</button>
						<button type='button' onClick={() => submitFeedback('negative')} title='Unhelpful response' className={`w-6 h-6 flex items-center justify-center rounded transition-colors ${feedback === 'negative' ? 'text-red-400 bg-red-500/10' : 'text-zinc-700 hover:text-zinc-400 hover:bg-zinc-800/50'}`}>
							<ThumbsDown size={11} />
						</button>
					</div>
				)}
...
export const ChatView: React.FC<ChatViewProps> = ({
	...
	const notify = useCallback((message: string) => {
		try { slashContext?.accessor.get(INotificationService).info(message); } catch { /* optional UI service */ }
	}, [slashContext]);
	...
	const copyText = useCallback(async (text: string) => {
		try {
			...
			console.warn('[Forge Chat] Clipboard write failed:', error);
			notify('Could not copy this message.');
		}
	}, [notify]);
	...
	const duplicateCurrentThread = useCallback(() => {
		...
		try {
			...
			slashContext.chatThreadsService.duplicateThread(threadId);
			notify('Thread duplicated.');
		} catch (error) {
			console.warn('[Forge Chat] Could not duplicate thread:', error);
		}
	}, [notify, slashContext]);
	...
	const recordFeedback = useCallback((message: ChatViewMessage, rating: 'positive' | 'negative') => {
		if (!slashContext) return;
		try {
			slashContext.accessor.get(IMetricsService).capture('Forge Assistant Response Feedback', {
				rating,
			...
			console.warn('[Forge Chat] Could not record response feedback:', error);
		}
	...
	const handleSend = useCallback(async () => {
		...
		const text = draftText.trim() || (effectiveAttachments.length > 0
			? 'Inspect the attached context and complete the requested work. Read relevant files first, make the necessary changes, and verify the result.'
			: '');
		if (!text) return;
	...
	const handleAbort = useCallback(async () => {
		if (!slashContext) return;
		try {
			await slashContext.chatThreadsService.abortRunning(slashContext.chatThreadsService.state.currentThreadId);
		} catch (error) {
			console.warn('[Forge Chat] Abort failed:', error);
			notify('Could not stop the current run.');
		}
	}, [notify, slashContext]);

	const handleAddAttachment = useCallback((attachment: NewAttachment) => {
		if (!slashContext) {
			notify('Attachment service is unavailable in this view.');
			return;
		}
		const uri = URI.file(attachment.uri);
		const isImage = attachment.mimeType.startsWith('image/');
		const selection: StagingSelectionItem = isImage
			? { type: 'Image', uri, dataUrl: attachment.dataUrl, mimeType: attachment.mimeType }
			: { type: 'File', uri, language: attachment.uri.split('.').pop() || '', state: { wasAddedAsCurrentFile: false } };
		slashContext.chatThreadsService.addNewStagingSelection(selection);
		setLocalAttachments(previous => previous.some(item => item.uri === attachment.uri)
			? previous
			: [...previous, { ...attachment, name: attachment.uri.split(/[\\/]/).pop() || attachment.uri }]);
	}, [notify, slashContext]);
	...
	const removeStagedAttachment = useCallback((attachment: Attachment) => {
		...
		const isImage = attachment.mimeType.startsWith('image/');
		const next = threadState.stagingSelections.filter(selection => {
			if (selection.uri.fsPath !== fsPath) return true;
			return isImage ? selection.type !== 'Image' : selection.type !== 'File';
		});
		if (next.length !== threadState.stagingSelections.length) {
			slashContext.chatThreadsService.setCurrentThreadState({ stagingSelections: next });
		}
	}, [slashContext]);
	...
	const handleOpenCommands = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
		setSlashAnchor(e.currentTarget.getBoundingClientRect());
		setIsSlashOpen(true);
	}, []);
						...
						{messages.map((message, index) => {
							...
							return (
								<AssistantMessage
									key={message.id}
									message={message}
									streamEvents={isLastAssistant ? streamEventsForLastResponse : []}
									isStreaming={isStreaming && isLastAssistant}
									onRevert={revert}
									onCopy={copy}
									onDuplicateThread={slashContext ? duplicateCurrentThread : undefined}
									onFeedback={slashContext ? rating => recordFeedback(message, rating) : undefined}
								/>
							);
						})}
