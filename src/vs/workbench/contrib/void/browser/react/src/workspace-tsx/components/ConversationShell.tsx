...
export const ConversationShell: React.FC = () => {
			...
			.map((message: any, index: number): ChatViewMessage | null => {
				...
				return {
					...
					timestamp: typeof message.timestamp === 'number'
						? message.timestamp
						: new Date(currentThread.lastModified || currentThread.createdAt).getTime(),
					messageIndex: index,
				};
			})
	...
	const threadItems = useMemo(() => {
		return Object.values(threadsState.allThreads)
			...
			.map(thread => {
				...
				return {
					id: thread.id,
					title: firstText.slice(0, 44) || 'New conversation',
	...
	}, [currentThreadId, threadsState.allThreads]);
	...
	return (
		<div className='flex h-full w-full min-h-0 min-w-0 overflow-hidden bg-void-bg-1'>
			<SimpleSidebar
				threads={threadItems}
