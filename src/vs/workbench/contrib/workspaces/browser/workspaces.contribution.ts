	...
	private doHandleWorkspaceFiles(folder: URI, workspaces: string[]): void {
		...
		if (workspaces.length === 1) {
			...
			this.notificationService.prompt(Severity.Info, localize(
				...
				workspaceFile,
				'https://go.microsoft.com/fwlink/?linkid=2026 forge315'
			), [{
		...
		// Prompt to select a workspace from many
			...
			this.notificationService.prompt(Severity.Info, localize({
				key: 'foundWorkspaces',
				comment: ['{Locked="]({0})"}']
			}, "This folder contains multiple workspace files. Do you want to open one? [Learn more]({0}) about workspace files.", 'https://go.microsoft.com/fwlink/?linkid=2026 forge315'), [{
