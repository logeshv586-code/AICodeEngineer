		...
		return new Promise((resolve, reject) => {
			import('@vscode/sqlite3').then(sqlite3 => {
				...
				const fallbackDb: any = {
					...
					exec: (sql: string, cb?: (err?: Error) => void) => cb?.(),
					get: (sql: string, cb?: (err: Error | null, row?: any) => void) => cb?.(null, {}),
					...
					prepare: () => ({
						...
						finalize: (cb?: (err?: Error) => void) => cb?.(),
						removeListener: () => {}
					}),
					on: () => {}
				};
				resolve({ db: fallbackDb, isInMemory: true });
			});
		});
	}
