...
export const runUiContractTest = () => {
  ...
  const checks = [
    check('conversation shell is active UI', contains(files.activeSidebar, ['ConversationShell', '<ConversationShell />']) && !files.activeSidebar.includes('<SidebarChat'), 'The rendered Forge sidebar must use the premium conversation shell instead of the legacy compact renderer'),
    ...
    check('missing model recovery', contains(files.chat, ['if (!settingsService.state.modelSelectionOfFeature.Chat)', 'Choose a Chat model before sending a task', 'onOpenSettings?.()']), 'A missing Chat model must open recovery settings instead of producing a silent provider failure'),
    ...
    check('responsive context rail', contains(files.context, ['forge-brand-context-panel', 'Quick actions']) && files.brandCss.includes('@media (max-width: 1180px)'), 'Context is useful on wide layouts and does not crush smaller panes'),
    check('super-agent sidebar controls', contains(files.sidebar, ['forge_browser', 'forge_understand', 'forge_workflow', "name: 'open-design'", 'forge_integrations']), 'Conversation sidebar controls must call the real Forge MCP server'),
