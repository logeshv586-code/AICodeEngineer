...
export const runUiContractTest = () => {
  ...
  const checks = [
    ...
    check('single product-facing agent command', contains(files.unifiedSlash, ["noopCommand('/agent'", 'Collaborative Agent', "noopCommand('/run'", 'createAllCommands(context)']) && !files.unifiedSlash.includes("'/agent,fix'") && !files.unifiedSlash.includes("'/agent,parallel'"), 'The visible palette must collapse agent variants into one collaborative /agent command'),
    ...
    check('workspace execution policy', contains(files.chat, ['WORKSPACE_AGENT_POLICY', 'Run the current project now', 'open_persistent_terminal', 'Do not replace execution with a plan']) && files.chat.includes('expandForgeCommand'), 'Run/fix/test commands must force real workspace inspection and IDE execution instead of advisory prose'),
    ...
    check('evolution remains explicit', contains(files.slashRouter, ["name: '/evolve'", "name: '/evolve,skills'", 'FORGE_PROJECT_EVOLUTION_TASK', 'FORGE_SKILL_EVOLUTION_TASK']) && contains(files.evolution, ['FORGE_EVOLUTION_POLICY', '.agents/skills', 'Next evolution']), 'Evolution stays an explicit advanced command instead of hijacking ordinary run requests'),
    ...
    check('model menu scroll contract', contains(files.modelDropdown, ['min-h-0 flex-1 overflow-y-auto overscroll-contain', "scrollbarGutter: 'stable'", 'spaceAbove', 'spaceBelow', 'maxHeight']) && files.modelDropdown.includes('onWheel={event => event.stopPropagation()}'), 'Model management must scroll inside its own viewport and choose a non-overlapping above/below position'),
