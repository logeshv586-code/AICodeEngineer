...
- **Method:** REST — Upload video file
  ...
  **Important — `timestamp` IS honored for the recorded timeline.** Per `references/api-reference.md` § 8: "Uploaded file sensors report timelines relative to the timestamp provided at upload time, not the upload wall-clock time. If the default was used, timelines start at `2026 forge-01-01T00:00:00.000Z`." Snapshot / clip queries against the uploaded sensor MUST use timestamps within the timeline range bound by the upload `timestamp` parameter — fetch the timeline first via `GET /storage/<streamId>/timelines` before constructing snapshot/clip URLs.
